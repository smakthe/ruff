...
mod tests {
    ...
    use crate::{
        ...
        session::manager::{Message, MessageMetadata, MessageRole},
        EnhancedError,
    };
    ...
    async fn test_session_creation_and_switching() {
        ...
        assert_eq!(
            app.get_session_manager().session_count(),
    ...
    async fn test_message_handling_integration() {
        ...
        let message_id = app
            .get_message_manager_mut()
            .add_message(session_id, test_message)
            .await
            .unwrap();

        // Verify message was added
        assert_eq!(
            app.get_message_manager().get_message_count(session_id),
            initial_message_count + 1
        );
        assert!(app
            .get_message_manager()
            .message_exists(session_id, message_id));

        // Verify message content
        let retrieved_message = app
            .get_message_manager()
    ...
    async fn test_search_integration() {
        ...
        let found_session = search_results
            .iter()
    ...
    async fn test_model_switching_integration() {
        ...
        if available_models.len() > 1 {
            ...
            let different_model = available_models
                .iter()
            ...
            if let Some(new_model) = different_model {
                ...
                match result {
                    ...
                    Err(e) if e.to_string().contains("InvalidApiKey") => {
