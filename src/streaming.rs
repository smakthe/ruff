...
use crate::{
    ...
    events::{AppEvent, EventBus, MessageId, SessionId},
...
impl StreamingService {
    ...
    pub async fn start_streaming(
        ...
        self.event_bus
            .publish(AppEvent::StreamingStarted {
                session_id,
                message_id,
            })
            .await
    ...
    pub async fn cancel_streaming(&self, message_id: MessageId) -> Result<(), EnhancedError> {
        ...
        if let Some(state) = streams.get_mut(&message_id) {
            ...
            self.event_bus
                .publish(AppEvent::StreamingCancelled {
            ...
            while let Some(chunk_result) = stream.next().await {
                match chunk_result {
                        ...
                        // Update streaming state
                        {
                            ...
                            if let Some(state) = streams.get_mut(&message_id) {
                                if !chunk.content.is_empty() {
                                    ...
                                    // Emit chunk event
                                    let _ = event_bus
                                        .publish(AppEvent::StreamingChunk {
                                            session_id,
                                            message_id,
                                ...
                                if chunk.is_complete {
                                    ...
                                    let _ = event_bus
                                        .publish(AppEvent::StreamingCompleted {
                                            session_id,
                                            message_id,
                                            token_usage,
                                        })
                    ...
                    Err(error) => {
                        // Emit error event
                        let _ = event_bus
                            .publish(AppEvent::StreamingError {
                                session_id,
                                message_id,
    ...
    pub async fn cleanup_streams(&self) {
        ...
        streams
            .retain(|_, state| !state.is_complete && state.started_at.elapsed() < stale_threshold);
    }
    ...
    pub async fn get_streaming_stats(&self) -> StreamingStats {
        ...
        let total_content_length: usize = streams.values().map(|state| state.content.len()).sum();

        StreamingStats {
            active_streams: active_count,
            total_content_length,
            average_response_time: streams
                .values()
