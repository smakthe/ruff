...
impl SessionManager {
        ...
        while let Some(entry) = entries.next_entry().await
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match self.load_session_from_file(&path).await {
                    ...
                    Err(e) => {
                        eprintln!("Warning: Failed to load session from {}: {}", path.display(), e);
                    }
                }
            }
        }
    ...
    pub async fn rename_session(&mut self, session_id: SessionId, new_title: String) -> Result<(), EnhancedError> {
        ...
        // Publish event
        self.event_bus.publish(AppEvent::SessionRenamed {
            id: session_id,
            old_title,
    ...
    pub fn get_session_tabs(&self) -> Vec<SessionTab> {
        let mut tabs: Vec<SessionTab> = self.sessions
            .values()
    ...
    pub fn get_session_statistics(&self) -> SessionStatistics {
        ...
        let total_tokens: u32 = self.sessions.values().map(|s| s.total_tokens_used.total_tokens).sum();
        
        let oldest_session = self.sessions.values()
            .min_by_key(|s| s.created_at)
            .map(|s| s.created_at);
        
        let most_recent_activity = self.sessions.values()
...
mod tests {
    ...
    async fn test_switch_session() {
        ...
        let session1_id = manager.create_session(Some("Session 1".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Session 2".to_string())).await.unwrap();
    ...
    async fn test_get_active_session() {
        ...
        let session_id = manager.create_session(Some("Active Session".to_string())).await.unwrap();
    ...
    async fn test_delete_session() {
        ...
        let session_id = manager.create_session(Some("To Delete".to_string())).await.unwrap();
        manager.switch_session(session_id).await.unwrap();
    ...
    async fn test_session_persistence() {
        ...
        let session_id = {
            ...
            let id = manager.create_session(Some("Persistent Session".to_string())).await.unwrap();
    ...
    async fn test_get_all_sessions() {
        ...
        let session1_id = manager.create_session(Some("Session 1".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Session 2".to_string())).await.unwrap();
        let session3_id = manager.create_session(Some("Session 3".to_string())).await.unwrap();
    ...
    async fn test_session_last_activity_update() {
        ...
        let session_id = manager.create_session(Some("Activity Test".to_string())).await.unwrap();
    ...
    async fn test_event_publishing() {
        ...
        let _handle = manager.event_bus.subscribe(move |event| {
            match event {
                AppEvent::SessionCreated(_) | AppEvent::SessionSwitched(_) | AppEvent::SessionDeleted(_) => {
        ...
        let session_id = manager.create_session(Some("Event Test".to_string())).await.unwrap();
        manager.switch_session(session_id).await.unwrap();
        manager.delete_session(session_id).await.unwrap();
    ...
    async fn test_save_all_sessions() {
        ...
        let _session1_id = manager.create_session(Some("Session 1".to_string())).await.unwrap();
        let _session2_id = manager.create_session(Some("Session 2".to_string())).await.unwrap();
    ...
    async fn test_mutable_session_access() {
        ...
        let session_id = manager.create_session(Some("Mutable Test".to_string())).await.unwrap();
        manager.switch_session(session_id).await.unwrap();
        ...
        assert_eq!(manager.get_active_session().unwrap().title, "Modified Title");
        ...
        assert_eq!(manager.get_session(session_id).unwrap().model, "openai-gpt4");
    }
    ...
    async fn test_session_search_by_title() {
        ...
        let _session1_id = manager.create_session(Some("Machine Learning Discussion".to_string())).await.unwrap();
        let _session2_id = manager.create_session(Some("Rust Programming Help".to_string())).await.unwrap();
        let _session3_id = manager.create_session(Some("Python Data Analysis".to_string())).await.unwrap();
    ...
    async fn test_session_search_fuzzy_matching() {
        ...
        let _session_id = manager.create_session(Some("JavaScript Development".to_string())).await.unwrap();
    ...
    async fn test_session_tag_management() {
        ...
        let session_id = manager.create_session(Some("Tagged Session".to_string())).await.unwrap();
        
        // Add tags
        manager.add_tag_to_session(session_id, "programming".to_string()).await.unwrap();
        manager.add_tag_to_session(session_id, "rust".to_string()).await.unwrap();
        ...
        // Remove a tag
        manager.remove_tag_from_session(session_id, "programming").await.unwrap();
    ...
    async fn test_session_search_by_tags() {
        ...
        let session1_id = manager.create_session(Some("Session 1".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Session 2".to_string())).await.unwrap();
        let _session3_id = manager.create_session(Some("Session 3".to_string())).await.unwrap();
        
        manager.add_tag_to_session(session1_id, "rust".to_string()).await.unwrap();
        manager.add_tag_to_session(session2_id, "python".to_string()).await.unwrap();
    ...
    async fn test_session_filter_by_model() {
        ...
        let _session1_id = manager.create_session(Some("GPT Session".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Claude Session".to_string())).await.unwrap();
    ...
    async fn test_session_filter_by_date_range() {
        ...
        let session1_id = manager.create_session(Some("Old Session".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("New Session".to_string())).await.unwrap();
    ...
    async fn test_get_all_tags_and_models() {
        ...
        manager.add_tag_to_session(session1_id, "rust".to_string()).await.unwrap();
        manager.add_tag_to_session(session1_id, "programming".to_string()).await.unwrap();
        manager.add_tag_to_session(session2_id, "python".to_string()).await.unwrap();
    ...
    async fn test_search_index_rebuild() {
        ...
        let session_id = manager.create_session(Some("Test Session".to_string())).await.unwrap();
        manager.add_tag_to_session(session_id, "test".to_string()).await.unwrap();
    ...
    async fn test_session_search_sorting() {
        ...
        let session1_id = manager.create_session(Some("Alpha Session".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Beta Session".to_string())).await.unwrap();
        let session3_id = manager.create_session(Some("Gamma Session".to_string())).await.unwrap();
    ...
    async fn test_session_renaming() {
        ...
        let session_id = manager.create_session(Some("Original Title".to_string())).await.unwrap();
        
        // Test successful rename
        manager.rename_session(session_id, "New Title".to_string()).await.unwrap();
        
        let session = manager.get_session(session_id).unwrap();
        assert_eq!(session.title, "New Title");
        
        // Test rename with whitespace trimming
        manager.rename_session(session_id, "  Trimmed Title  ".to_string()).await.unwrap();
    ...
    async fn test_session_rename_validation() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("cannot exceed 200 characters"));
    }
    ...
    async fn test_session_archiving() {
        ...
        let session_id = manager.create_session(Some("Archive Test".to_string())).await.unwrap();
        manager.switch_session(session_id).await.unwrap();
    ...
    async fn test_get_archived_and_active_sessions() {
        ...
        let session1_id = manager.create_session(Some("Active Session".to_string())).await.unwrap();
        let session2_id = manager.create_session(Some("Archived Session".to_string())).await.unwrap();
    ...
    async fn test_get_session_metadata() {
        ...
        let session_id = manager.create_session(Some("Metadata Session".to_string())).await.unwrap();
            ...
            .filter(|session| {
                ...
                if let Some(filter_pattern) = filter {
                    ...
                    return session.title.to_lowercase().contains(&pattern_lower) ||
                           session.tags.iter().any(|tag| tag.to_lowercase().contains(&pattern_lower));
                }
