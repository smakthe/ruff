...
impl SessionSearchIndex {
    ...
    pub fn search(&self, query: &SessionSearchQuery) -> Vec<SessionSearchResult> {
        ...
        for indexed_session in self.indexed_sessions.values() {
            ...
            if let Some((score, match_type)) =
                self.calculate_relevance(indexed_session, &query.text)
            {
        ...
        for indexed_session in self.indexed_sessions.values() {
            ...
            let has_matching_tag = tags.iter().any(|tag| {
                indexed_session
                    .tags
                    .iter()
                    .any(|session_tag| session_tag.to_lowercase().contains(&tag.to_lowercase()))
            });
        ...
        if let Some(filter_tags) = &filters.tags {
            let has_matching_tag = filter_tags.iter().any(|filter_tag| {
                session.tags.iter().any(|session_tag| {
                    session_tag
                        .to_lowercase()
                        .contains(&filter_tag.to_lowercase())
                })
            });
            if !has_matching_tag {
                return false;
    ...
    fn calculate_relevance(
        ...
        if let Some(score) = self
            .matcher
            .fuzzy_match(&session.content_preview, &search_text)
        {
