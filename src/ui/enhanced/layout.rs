...
impl ResizablePane {
    ...
    pub fn resize(&mut self, delta_width: i16, delta_height: i16) -> Result<(), EnhancedError> {
        ...
        let new_width = (self.current_size.width as i16 + delta_width)
            .max(self.config.min_size.width as i16) as u16;
        let new_height = (self.current_size.height as i16 + delta_height)
            .max(self.config.min_size.height as i16) as u16;
...
impl LayoutManager {
    ...
    pub fn handle_mouse_event(&mut self, mouse: MouseEvent) -> Result<bool, EnhancedError> {
        match mouse.kind {
            ...
            MouseEventKind::Drag(MouseButton::Left) => {
                if let (Some(target_id), Some(last_pos)) =
                    (&self.resize_target, self.last_mouse_pos)
                {
    ...
    pub fn save_layout(&self) -> Result<(), EnhancedError> {
        if let Some(config_path) = &self.config_file_path {
            ...
            std::fs::write(config_path, config_json).map_err(|e| {
                EnhancedError::storage(format!("Failed to write layout config: {}", e))
            })?;
        }
        Ok(())
    }
        ...
        if let Some(config_path) = &self.config_file_path {
            if config_path.exists() {
                let config_json = std::fs::read_to_string(config_path).map_err(|e| {
                    EnhancedError::storage(format!("Failed to read layout config: {}", e))
                })?;

                let loaded_config: LayoutConfig =
                    serde_json::from_str(&config_json).map_err(|e| {
                        EnhancedError::unknown(format!("Failed to parse layout config: {}", e))
                    })?;
    ...
    pub fn get_layout_stats(&self) -> HashMap<String, serde_json::Value> {
        ...
        stats.insert(
            ...
            serde_json::Value::Number(self.panes.len().into()),
        );
        stats.insert(
            "font_size".to_string(),
            serde_json::Value::Number(self.layout_config.font_size.into()),
        );
        stats.insert(
            "resize_mode".to_string(),
            serde_json::Value::Bool(self.resize_mode),
        );
        stats.insert(
            "focused_pane".to_string(),
...
mod tests {
    ...
    fn test_resize_direction_constraints() {
        ...
        let vertical_config =
            PaneConfig::new("v_pane".to_string()).with_resize_direction(ResizeDirection::Vertical);
    ...
    fn test_layout_stats() {
        ...
        assert_eq!(
            stats.get("focused_pane").unwrap().as_str().unwrap(),
            "pane1"
        );
    }
