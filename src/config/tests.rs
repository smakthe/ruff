...
async fn test_global_config_crud() {
    ...
    config
        .api_keys
        .insert("openai".to_string(), "sk-valid-key-123".to_string());
    config
        .api_keys
        .insert("anthropic".to_string(), "sk-ant-valid-key-123".to_string());
    config
        .api_keys
        .insert("groq".to_string(), "gsk_valid-key-123".to_string());
    config
        .api_keys
        .insert("huggingface".to_string(), "hf_valid-key-123".to_string());
    config
        .api_keys
        .insert("cohere".to_string(), "valid-cohere-key-123".to_string());
    config
        .api_keys
        .insert("together".to_string(), "valid-together-key-123".to_string());
...
async fn test_model_config_crud() {
    ...
    assert_eq!(
        retrieved_config.custom_endpoint,
        test_config.custom_endpoint
    );
    assert_eq!(
        retrieved_config.rate_limit.requests_per_minute,
        test_config.rate_limit.requests_per_minute
    );
    assert_eq!(
        retrieved_config.retry_config.max_attempts,
        test_config.retry_config.max_attempts
    );
...
async fn test_theme_config_crud() {
    ...
    // Create
    service
        .update_theme_config("test-theme", test_theme.clone())
        .await
        .unwrap();
...
async fn test_plugin_config_crud() {
    ...
    // Create
    service
        .update_plugin_config("test-plugin", test_plugin.clone())
        .await
        .unwrap();
...
async fn test_api_key_management() {
    ...
    // Set API key
    service
        .set_api_key("test-provider", "test-api-key-123")
        .await
        .unwrap();
    ...
    service
        .set_api_key("placeholder-provider", "your-api-key-here")
        .await
        .unwrap();
...
async fn test_config_validation() {
    ...
    assert!(service
        .update_model_config("invalid", invalid_model_config)
        .await
        .is_err());
    ...
    assert!(service
        .update_theme_config("invalid", invalid_theme_config)
        .await
        .is_err());
    ...
    assert!(service
        .update_global_config(invalid_global_config)
        .await
        .is_err());
}
...
async fn test_validate_all_configs() {
    ...
    service
        .set_api_key("openai", "sk-valid-api-key-123")
        .await
        .unwrap();

    // Should pass validation with valid configs
    let _result = service.validate_all_configs();
...
async fn test_config_persistence() {
    ...
    // Create service and add some configs
    {
        ...
        service
            .update_model_config("persistent-model", test_config)
            .await
            .unwrap();
        ...
        service
            .update_theme_config("persistent-theme", test_theme)
            .await
            .unwrap();
    }
...
async fn test_export_import_config() {
    ...
    service
        .update_model_config("export-test-model", test_model)
        .await
        .unwrap();
    service
        .set_api_key("export-test", "export-api-key")
        .await
        .unwrap();
    ...
    new_service
        .import_config(&export_path, false)
        .await
        .unwrap();
...
async fn test_import_config_merge() {
    ...
    // Set initial API key
    service
        .set_api_key("initial-provider", "initial-key")
        .await
        .unwrap();
