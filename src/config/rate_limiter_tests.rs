...
async fn test_rate_limiter_integration() {
    ...
    service.update_model_config("rate-limited-model", model_config).await.unwrap();
...
async fn test_retry_handler_integration() {
    ...
    service.update_model_config("retry-model", model_config).await.unwrap();
...
async fn test_rate_limit_status_all_providers() {
    ...
    for (i, (provider, rpm, tpm)) in providers.iter().enumerate() {
        ...
        service.update_model_config(&format!("model-{}", i), model_config).await.unwrap();
    }
    ...
    for (provider, expected_rpm, expected_tpm) in providers {
        let status = all_statuses.iter()
            .find(|s| s.provider == provider)
            .expect(&format!("Provider '{}' not found in rate limit statuses", provider));
...
async fn test_wait_for_request_integration() {
    ...
    service.update_model_config("fast-model", model_config).await.unwrap();
    
    // Should be able to wait for request immediately
    let start = std::time::Instant::now();
    service.wait_for_request("fast-provider", Some(1000)).await.unwrap();
...
async fn test_rate_limiter_with_very_low_limits() {
    ...
    service.update_model_config("slow-model", model_config).await.unwrap();
...
async fn test_concurrent_request_limiting() {
    ...
    service.update_model_config("concurrent-model", model_config).await.unwrap();
    ...
    let _slot1 = {
        ...
        rate_limiter.acquire_request_slot("concurrent-provider").unwrap()
    };
    ...
    {
        let rate_limiter = rate_limiter.read().unwrap();
        assert!(rate_limiter.acquire_request_slot("concurrent-provider").is_err());
    }
    ...
    {
        let rate_limiter = rate_limiter.read().unwrap();
        assert!(rate_limiter.acquire_request_slot("concurrent-provider").is_ok());
    }
}
...
async fn test_retry_handler_with_different_errors() {
    ...
    service.update_model_config("retry-test-model", model_config).await.unwrap();
...
async fn test_model_config_update_syncs_rate_limits() {
    ...
    service.update_model_config("sync-test-model", initial_config).await.unwrap();
    ...
    service.update_model_config("sync-test-model", updated_config).await.unwrap();
...
async fn test_rate_limit_status_nonexistent_provider() {
    ...
    assert!(service.get_rate_limit_status("non-existent-provider").is_none());
}
