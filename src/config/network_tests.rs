...
async fn test_custom_endpoint_management() {
    ...
    // Set custom endpoint
    service.set_custom_endpoint(model_name, custom_endpoint).unwrap();
...
async fn test_invalid_custom_endpoint() {
    ...
    // Should fail with invalid URL
    assert!(service.set_custom_endpoint("test-model", "not-a-url").is_err());
    
    // Should fail with non-HTTP(S) URL
    assert!(service.set_custom_endpoint("test-model", "ftp://example.com").is_err());
}
...
async fn test_effective_endpoint_resolution() {
    ...
    service.update_model_config("test-model", model_config).await.unwrap();
    ...
    service.set_custom_endpoint("test-model", "https://custom.api.com/v1").unwrap();
    let endpoint = service.get_effective_endpoint("test-model").unwrap();
    ...
    service.update_model_config("test-model", model_config_with_custom).await.unwrap();
...
async fn test_effective_endpoint_nonexistent_model() {
    ...
    assert!(service.get_effective_endpoint("non-existent-model").is_err());
}
...
async fn test_connection_testing() {
    ...
    let result = service.test_custom_endpoint("https://nonexistent.example.com").await;
...
fn test_default_provider_endpoints() {
    ...
    assert_eq!(network_config.get_default_endpoint("openai"), "https://api.openai.com/v1");
    assert_eq!(network_config.get_default_endpoint("anthropic"), "https://api.anthropic.com/v1");
    assert_eq!(network_config.get_default_endpoint("cohere"), "https://api.cohere.ai/v1");
    assert_eq!(network_config.get_default_endpoint("together"), "https://api.together.xyz/v1");
    assert_eq!(network_config.get_default_endpoint("groq"), "https://api.groq.com/openai/v1");
    assert_eq!(network_config.get_default_endpoint("huggingface"), "https://api-inference.huggingface.co/models");
    
    // Test unknown provider gets generic endpoint
    assert_eq!(network_config.get_default_endpoint("unknown"), "https://api.unknown.com/v1");
}
...
async fn test_model_config_with_proxy() {
    ...
    assert!(service.update_model_config("proxy-model", model_config.clone()).await.is_ok());
    ...
    assert_eq!(retrieved_config.proxy_config.unwrap().url, "http://model-proxy.example.com:8080");
}