...
pub enum AppEvent {
    ...
    ImportCompleted {
        imported_sessions: Vec<SessionId>,
    },
    SessionImported {
        session_id: SessionId,
    ...
    UIExtensionRegistered {
        plugin_id: PluginId,
        extension_id: String,
    },
    UIExtensionUnregistered {
        plugin_id: PluginId,
        extension_id: String,
    },
...
pub type EventHandler =
    Arc<dyn Fn(&AppEvent) -> Result<(), Box<dyn std::error::Error + Send + Sync>> + Send + Sync>;
...
impl EventBus {
    ...
    where
        F: Fn(&AppEvent) -> Result<(), Box<dyn std::error::Error + Send + Sync>>
            + Send
            + Sync
            + 'static,
    {
    ...
    {
        let handle = SubscriptionHandle(Uuid::new_v4());
        let async_handler: AsyncEventHandler = Arc::new(move |event| Box::pin(handler(event)));
    ...
    pub async fn start_processing(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let mut receiver = {
            ...
            receiver_guard
                .take()
                .ok_or("Event processing already started")?
        };
