...
impl TemplateManager {
    ...
    /// Search templates
    pub async fn search_templates(&self, query: &str, filters: Option<TemplateSearchFilters>) -> Vec<TemplateSearchResult> {
        let templates = self.templates.read().await;
    ...
    pub async fn get_templates_by_category(&self, category: TemplateCategory) -> Vec<ConversationTemplate> {
        let templates = self.templates.read().await;
        templates.values()
    ...
    pub async fn apply_template(&self, template_id: TemplateId, variables: Vec<TemplateVariable>) -> Result<AppliedTemplate, RuffError> {
        ...
        let var_map: HashMap<String, String> = variables.into_iter()
            .map(|var| (var.name, var.value))
            .collect();
        
        // Apply substitutions
        let system_prompt = template.system_prompt.as_ref()
            .map(|prompt| self.substitute_variables(prompt, &var_map));
        
        let initial_messages: Vec<_> = template.initial_messages.iter()
    ...
    pub async fn get_template_stats(&self) -> TemplateStats {
        ...
        for template in templates.values() {
            *category_counts.entry(template.category.clone()).or_insert(0) += 1;
        }
        
        let most_used = templates.values()
...
mod tests {
    ...
    async fn test_template_categories() {
        ...
        let dev_templates = manager.get_templates_by_category(TemplateCategory::Development).await;
    ...
    async fn test_apply_template_with_variables() {
        ...
        let applied = manager.apply_template(template_id, variables).await.unwrap();
        
        assert_eq!(applied.system_prompt, Some("You are helping Alice".to_string()));
        assert_eq!(applied.initial_messages[0].content, "Hello, my name is Alice and I work in technology");
    }
    ...
    async fn test_export_import_templates() {
        ...
        let imported_ids = manager2.import_templates(&export_path, false).await.unwrap();
