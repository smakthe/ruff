...
use crate::{
    ...
    ui::enhanced::{
        LayoutManager, layout::{PaneConfig, ResizeDirection},
...
fn render_header_with_theme(f: &mut Frame, area: Rect, session: &crate::session::manager::ChatSession, current_model: &AIModel, font_size: u16, theme_colors: &crate::ui::enhanced::ThemeColors) {
    let title = format!("🦀 Ruff - {} | Model: {} | Session: {} | Font: {}pt", 
        current_model.name, 
        current_model.provider,
        session.title.chars().take(25).collect::<String>(),
...
fn render_input_with_theme(f: &mut Frame, area: Rect, input_buffer: &str, theme_colors: &crate::ui::enhanced::ThemeColors) {
    let input_text = if input_buffer.is_empty() {
        "Type your message... (Ctrl+Shift+P commands, F1 help, Ctrl+N new session, Ctrl+C quit)".to_string()
    } else {
        input_buffer.to_string()
    };
...
fn render_status_with_theme(f: &mut Frame, area: Rect, session: &crate::session::manager::ChatSession, font_size: u16, theme_colors: &crate::ui::enhanced::ThemeColors) {
    let session_id_short = &session.id.to_string()[..8];
    let status_text = format!(
...
fn render_model_selector_with_theme(f: &mut Frame, model_registry: &ModelRegistry, selected_model_index: usize, theme_colors: &crate::ui::enhanced::ThemeColors) {
    let area = centered_rect(60, 70, f.area());
    ...
    let models: Vec<ListItem> = model_registry
        ...
        .map(|(i, (_key, model))| {
            let style = if i == selected_model_index {
                Style::default().bg(theme_colors.primary).fg(theme_colors.on_primary)
            } else {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(format!("{:<15}", model.provider), Style::default().fg(provider_color)),
                Span::styled(format!("{:<25}", model.name), Style::default().fg(theme_colors.on_surface)),
                Span::styled(format!("{}K tokens", model.max_tokens / 1000), Style::default().fg(theme_colors.on_surface_variant)),
            ])).style(style)
        })
        .collect();
    ...
    let model_list = List::new(models)
        .block(Block::default()
            ...
            .title_style(Style::default().fg(theme_colors.primary))
            .border_style(Style::default().fg(theme_colors.outline)))
        .highlight_style(Style::default().bg(theme_colors.primary_container).fg(theme_colors.on_primary_container));
        
    f.render_widget(model_list, area);
}

fn format_message<'a>(message: &'a crate::session::manager::Message, markdown_renderer: &'a MarkdownRenderer) -> ListItem<'a> {
    ...
    let content_lines: Vec<Line> = if message.role == crate::session::manager::MessageRole::Assistant {
        // Use markdown rendering for AI responses
        let rendered = markdown_renderer.render(&message.content);
        rendered.text.lines.into_iter().map(|line| {
...
impl UI {
    ...
    pub fn render(&mut self, session: &crate::session::manager::ChatSession, messages: &[crate::session::manager::Message], current_model: &AIModel) -> Result<(), EnhancedError> {
        ...
        let extension_layout = if let Some(extension_manager) = &self.ui_extension_manager {
            // Use async runtime to calculate layout
        ...
        self.terminal.draw(move |f| {
            ...
            if let Some(ref ext_layout) = extension_layout {
                if let Some(ref manager) = extension_manager {
                    // Use async runtime to render extensions
                    if let Ok(rt) = tokio::runtime::Handle::try_current() {
                        let _ = rt.block_on(async {
                            manager.render_extensions(f, ext_layout).await
                        });
                    }
                }
            }
            ...
            let header_area = pane_areas.get("header").copied().unwrap_or_else(|| {
                Rect { x: main_content_area.x, y: main_content_area.y, width: main_content_area.width, height: 3 }
            });
            
            let messages_area = pane_areas.get("messages").copied().unwrap_or_else(|| {
                Rect { 
            ...
            let input_area = pane_areas.get("input").copied().unwrap_or_else(|| {
                Rect { 
                    x: main_content_area.x, 
            ...
            let status_area = pane_areas.get("status").copied().unwrap_or_else(|| {
                Rect { 
                    x: main_content_area.x, 
            ...
            if show_model_selector {
                render_model_selector_with_theme(f, &model_registry, selected_model_index, &theme_colors);
            }
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) -> UIAction {
        // First, let plugin UI extensions handle the event
        if let Some(extension_manager) = &self.ui_extension_manager {
            let event = crossterm::event::Event::Key(key);
            
            // Use async runtime to handle input
            if let Ok(rt) = tokio::runtime::Handle::try_current() {
                if let Ok(handled) = rt.block_on(async {
                    extension_manager.handle_input(&event).await
                }) {
                    if handled {
        ...
        if self.show_model_selector {
            ...
            match key.code {
                ...
                KeyCode::Char('p') if key.modifiers.contains(event::KeyModifiers::CONTROL | event::KeyModifiers::SHIFT) => {
                    self.command_palette.show();
                    UIAction::ShowCommandPalette
                }
                KeyCode::F(1) | KeyCode::Char('?') if key.modifiers.contains(event::KeyModifiers::CONTROL) => {
                ...
                KeyCode::Char('f') if key.modifiers.contains(event::KeyModifiers::CONTROL | event::KeyModifiers::SHIFT) => {
                ...
                KeyCode::Left if key.modifiers.contains(event::KeyModifiers::ALT) => {
                    // Navigate to previous session - placeholder
                    UIAction::None
                }
                KeyCode::Right if key.modifiers.contains(event::KeyModifiers::ALT) => {
                    // Navigate to next session - placeholder
                    UIAction::None
                }
                KeyCode::Char('+') | KeyCode::Char('=') if key.modifiers.contains(event::KeyModifiers::CONTROL) => {
                    // Increase font size
                    let current_size = self.layout_manager.get_font_size();
                    if let Ok(()) = self.layout_manager.set_font_size(current_size + 1) {
                        let _ = self.layout_manager.save_layout();
                        UIAction::FontSizeChanged(current_size + 1)
                    } else {
                        UIAction::None
                    }
                }
                ...
                KeyCode::Char('0') if key.modifiers.contains(event::KeyModifiers::CONTROL) => {
                    // Reset font size to default
                    if let Ok(()) = self.layout_manager.set_font_size(14) {
                        let _ = self.layout_manager.save_layout();
                        UIAction::FontSizeChanged(14)
                    } else {
                        UIAction::None
                    }
                }
    ...
    pub fn get_ui_extension_manager(&self) -> &Option<std::sync::Arc<crate::plugin::UIExtensionManager>> {
    ...
    pub fn set_ui_extension_manager(&mut self, manager: std::sync::Arc<crate::plugin::UIExtensionManager>) {
...
fn render_command_palette(f: &mut Frame, command_palette: &CommandPalette, theme_colors: &crate::ui::enhanced::ThemeColors) {
    ...
    let commands: Vec<ListItem> = command_palette
        ...
        .map(|(i, result)| {
            let style = if i == command_palette.selected_index() {
                Style::default().bg(theme_colors.primary_container).fg(theme_colors.on_primary_container)
            } else {
                Style::default().fg(theme_colors.on_surface)
            };
            
            let shortcut_text = result.command.shortcut
                .as_ref()
                .map(|s| format!(" ({})", s))
                .unwrap_or_default();
            ...
            let line = Line::from(vec![
                ...
                Span::styled(shortcut_text, Style::default().fg(theme_colors.on_surface_variant)),
            ]);
    ...
    let command_list = List::new(commands)
        .block(Block::default()
            ...
            .title_style(Style::default().fg(theme_colors.primary))
            .border_style(Style::default().fg(theme_colors.outline)))
        .highlight_style(Style::default().bg(theme_colors.primary).fg(theme_colors.on_primary));
        
    f.render_widget(command_list, chunks[1]);
}

fn render_help_system(f: &mut Frame, help_system: &HelpSystem, theme_colors: &crate::ui::enhanced::ThemeColors) {
    ...
    let tabs = Paragraph::new(Text::from(tab_titles))
        .block(Block::default()
            .borders(Borders::ALL)
            .title("Help")
            .title_style(Style::default().fg(theme_colors.primary))
...
fn render_shortcuts_help(f: &mut Frame, area: Rect, help_system: &HelpSystem, theme_colors: &crate::ui::enhanced::ThemeColors) {
...
fn render_general_help(f: &mut Frame, area: Rect, help_system: &HelpSystem, theme_colors: &crate::ui::enhanced::ThemeColors) {
            ...
            for item in items {
                ...
                for subsection in &item.subsections {
                    ...
                    lines.push(Line::from(Span::styled(
                        ...
                        Style::default().fg(theme_colors.secondary).add_modifier(Modifier::BOLD)
                    )));
