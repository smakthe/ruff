...
async fn test_streaming_service_stream_tracking() {
    ...
    assert!(streaming_service
        .get_streaming_content(message_id)
        .await
        .is_none());
}
...
async fn test_unsupported_streaming_model() {
    ...
    let result = client
        .send_streaming_message(&model, messages, "fake-key", 100, 0.7)
        .await;
    ...
    if let Err(error) = result {
        ...
        assert!(message.contains("streaming not supported"));
    } else {
