...
use crate::error::{
    EnhancedError, ErrorCategory, ErrorRecoveryManager, ErrorSeverity, RecoveryAction,
    RecoveryActionType,
};
use crate::events::EventBus;
use crate::logging::{ConsoleFormat, ConsoleOutput, LogLevel, LoggerConfig, StructuredLogger};
use crate::plugin::{IsolationConfig, PluginErrorEvent, PluginErrorIsolation};
...
mod enhanced_error_tests {
    ...
    fn test_enhanced_error_builder_pattern() {
        ...
        assert_eq!(
            error.user_message,
            Some("Something went wrong with your session".to_string())
        );
    ...
    fn test_error_serialization() {
        ...
        assert_eq!(
            ...
            Some("Invalid configuration value".to_string())
        );
    }
}
...
mod plugin_isolation_tests {
    ...
    async fn test_plugin_success_reporting() {
        ...
        let result = isolation
            .report_success(&plugin_id, Duration::from_millis(200))
            .await;
        assert!(result.is_ok());
...
mod logging_integration_tests {
    use super::*;
    use crate::logging::{ConsoleFormat, ConsoleOutput, LevelFilter};
    ...
    fn test_performance_logging() {
        ...
        assert_eq!(
            metrics.operation_timings.get("test_operation"),
            Some(&vec![150])
        );
    }
