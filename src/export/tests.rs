...
mod export_tests {
    ...
    use crate::export::formats::{
        FormatHandler, HtmlHandler, JsonHandler, MarkdownHandler, PlainTextHandler,
    };
...
mod import_tests {
    ...
    async fn test_detect_chatgpt_format() {
        ...
        let format = service
            .detect_format(&content, std::path::Path::new("test.json"))
            .unwrap();
    ...
    async fn test_import_chatgpt_conversation() {
        ...
        let result = service
            .import_from_content(&content, None, options)
            .await
            .unwrap();
    ...
    async fn test_import_from_file() {
        ...
        let options = ImportOptions::default();
        let result = service
            .import_from_file(&file_path, None, options)
            .await
            .unwrap();
    ...
    async fn test_import_invalid_json() {
        ...
        let options = ImportOptions::default();
        let result = service
            .import_from_content(&invalid_content, None, options)
            .await;
        ...
        assert!(
            error_msg.contains("Unable to detect")
    ...
    async fn test_import_empty_content() {
        ...
        let options = ImportOptions::default();
        let result = service
            .import_from_content(&empty_content, None, options)
            .await
            .unwrap();
    ...
    fn test_content_quality_validation() {
        ...
        assert!(warnings
            .iter()
            .any(|w| w.contains("dominated by repeated character")));
    }
    ...
    async fn test_chatgpt_complex_conversation() {
        ...
        let options = ImportOptions::default();
        let result = service
            .import_from_content(&complex_export, None, options)
            .await
            .unwrap();
...
mod bulk_operations_tests {
    ...
    async fn test_bulk_export_with_progress() {
        ...
        let result = export_service
            .export_sessions_with_progress(&sessions, request, Some(progress_callback))
            .await
            .unwrap();
    ...
    async fn test_bulk_export_with_errors() {
        ...
        let result = export_service
            .export_sessions_with_progress(&sessions, request, None)
            .await
            .unwrap();
    ...
    async fn test_bulk_import_from_directory() {
        ...
        export_service
            .export_sessions(&sessions, export_request)
            .unwrap();

        // Now test importing from the directory
        let import_result = export_service
            .import_sessions_from_directory(&temp_dir.path().join("exports"), None)
            .await
            .unwrap();

        println!(
            "Import result: total={}, successful={}, failed={}, errors={:?}",
            import_result.total_items,
            import_result.successful_items,
            import_result.failed_items,
    ...
    async fn test_bulk_import_with_progress() {
        ...
        let import_result = export_service
            .import_sessions_from_directory(
    ...
    async fn test_bulk_import_with_invalid_files() {
        ...
        let import_result = export_service
            .import_sessions_from_directory(&import_dir, None)
            .await
            .unwrap();
    ...
    fn test_extract_session_id_from_path() {
        ...
        assert_eq!(
            export_service.extract_session_id_from_path(&path1),
            Some(session_id)
        );
        ...
        let path2 = temp_dir
            .path()
            .join(format!("20240101_120000_Test_Session_{}.json", session_id));
        assert_eq!(
            export_service.extract_session_id_from_path(&path2),
            Some(session_id)
        );
...
mod backup_tests {
    ...
    async fn test_backup_config_validation() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Max backups must be greater than 0"));
    }
    ...
    async fn test_create_backup() {
        ...
        let result = backup_manager
            .create_backup(&sessions, Some(progress_callback))
            .await
            .unwrap();
    ...
    async fn test_list_backups() {
        ...
        // Create a backup
        let sessions = vec![create_test_session_with_id(
            uuid::Uuid::new_v4(),
            "Test Session",
        )];
        let result = backup_manager.create_backup(&sessions, None).await.unwrap();
    ...
    async fn test_delete_backup() {
        ...
        // Delete the backup
        backup_manager
            .delete_backup(result.metadata.backup_id)
            .await
            .unwrap();
