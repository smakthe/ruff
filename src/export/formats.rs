    ...
    fn export_messages(&self, messages: &[Message], options: &ExportOptions) -> Result<String, Box<dyn std::error::Error>> {
        ...
        for message in messages {
            ...
            if options.include_timestamps {
                output.push_str(&format!(" *({})* ", message.timestamp.format("%Y-%m-%d %H:%M:%S")));
            }
            ...
            if options.include_metadata {
                ...
                if message.metadata.response_time_ms > 0 {
                    metadata_parts.push(format!("Response time: {}ms", message.metadata.response_time_ms));
                }
                
                if message.metadata.is_regenerated {
                    metadata_parts.push(format!("Regenerated {} times", message.metadata.regeneration_count));
                }
...
impl FormatHandler for HtmlHandler {
    ...
    fn export_messages(&self, messages: &[Message], options: &ExportOptions) -> Result<String, Box<dyn std::error::Error>> {
        ...
        for message in messages {
            ...
            output.push_str(&format!("                <span class=\"role\">{} {}</span>\n", role_icon, role_class.to_uppercase()));
            
            if options.include_timestamps {
                output.push_str(&format!("                <span class=\"timestamp\">{}</span>\n", message.timestamp.format("%Y-%m-%d %H:%M:%S")));
            }
            
            if options.include_model_info && matches!(message.role, MessageRole::Assistant) {
                output.push_str(&format!("                <span class=\"model\">{}</span>\n", html_escape(&message.metadata.model_used)));
            }
            ...
            if options.include_metadata {
                ...
                if !metadata_parts.is_empty() {
                    ...
                    output.push_str(&format!("                <small>{}</small>\n", html_escape(&metadata_parts.join(" • "))));
                    output.push_str("            </div>\n");
                }
            }
