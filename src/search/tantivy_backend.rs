...
use tantivy::{Index, IndexWriter, IndexReader, TantivyDocument};
use tantivy::collector::TopDocs;
use tantivy::query::{QueryParser, Query, BooleanQuery, TermQuery, Occur};
...
use crate::message::search::{MessageSearchQuery, MessageSearchResult, SearchRanking};
...
impl TantivyMessageSearchIndex {
    ...
    pub fn new(index_path: PathBuf) -> Result<Self, EnhancedError> {
        ...
        std::fs::create_dir_all(&index_path)
            .map_err(|e| EnhancedError::storage(format!("Failed to create index directory: {}", e)))?;
        ...
        let index = if Index::exists(&dir)
            .map_err(|e| EnhancedError::storage(format!("Failed to check index existence: {}", e)))? {
            Index::open(dir)
    ...
    pub async fn remove_session(
        ...
        writer.delete_term(term);
        writer.commit()
            .map_err(|e| EnhancedError::storage(format!("Failed to commit session deletion: {}", e)))?;
    ...
    fn doc_to_search_result(
        ...
        Ok(Some(MessageSearchResult {
            ...
            message_id,
            content_snippet,
            full_content: if query.include_metadata { Some(content) } else { None },
    ...
    pub fn get_statistics(&self) -> Result<IndexStatistics, EnhancedError> {
        ...
        let total_documents: usize = segment_metas.iter()
...
impl crate::search::unified::SearchBackend for TantivyMessageSearchIndex {
    async fn index_session(&self, _session: &crate::session::manager::ChatSession) -> Result<(), EnhancedError> {
        // For now, we don't index session metadata separately
        // This can be extended in the future to index session titles, tags, etc.
        // The session metadata is already accessible through the session ID in messages
        Ok(())
    }

    async fn index_message(&self, session_id: SessionId, message: &crate::session::manager::Message) -> Result<(), EnhancedError> {
    ...
    async fn search(&self, query: &crate::search::unified::UnifiedSearchQuery) -> Result<Vec<crate::search::unified::SearchResult>, EnhancedError> {
        // For now, only implement message search
        // Session search can be added later
        match query.scope {
            crate::search::unified::SearchScope::Messages | crate::search::unified::SearchScope::Both => {
                ...
                let unified_results = results.into_iter()
                    .map(|r| {
            ...
            crate::search::unified::SearchScope::Sessions => {
                // Session-only search not yet implemented
                Ok(vec![])
            }
        }
    }
